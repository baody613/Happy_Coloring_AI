import { db as firestoreDb } from '../src/config/firebase.js';
...
// Sử dụng db instance đã được khởi tạo sẵn
...
async function seedProducts() {
  try {
    console.log('🌱 Bắt đầu seed products vào Firestore...\n');
    ...
    if (!oldProducts.empty) {
      console.log(`🗑️  Đang xóa ${oldProducts.size} products cũ...`);
    ...
    products.forEach((product, index) => {
      ...
      console.log(`✅ [${index + 1}/12] ${product.title}`);
    });

    await newBatch.commit();
    
    console.log('\n🎉 Seed thành công!');
    console.log(`📊 Đã thêm ${products.length} products vào Firestore`);
    console.log('\n📋 Chi tiết:');
