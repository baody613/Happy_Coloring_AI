...
const generateResponse = async (intent, message, keywords) => {
  switch (intent) {
    case "greeting":
      return {
        text: "Xin chào! 👋 Tôi là trợ lý của Yu Ling Store. Tôi có thể giúp bạn:\n\n" +
    ...
    case "category_inquiry":
      ...
      return {
        text: `📂 **Các danh mục tranh hiện có:**\n\n` +
    ...
    case "ai_feature":
      return {
        text: "✨ **Tạo Tranh AI - Độc Đáo Riêng Của Bạn**\n\n" +
    ...
    case "product_search":
      ...
      if (searchResults.length === 0) {
        return {
          text: "😅 Xin lỗi, tôi không tìm thấy sản phẩm phù hợp.\n\n" +
            "Bạn có thể:\n" +
            "• Thử từ khóa khác\n" +
            "• Xem tất cả sản phẩm\n" +
      ...
      return {
        text: `🎨 Tôi tìm được ${searchResults.length} sản phẩm phù hợp với "${message}":\n\n` +
    ...
    default:
      return {
        text: "🤔 Tôi chưa hiểu rõ câu hỏi của bạn.\n\n" +
  ...
  try {
    ...
    if (filters.keywords && filters.keywords.length > 0) {
      return products.filter((p) => {
        ...
        return filters.keywords.some((kw) => searchText.includes(kw.toLowerCase()));
      });
    }
...
const getPriceRanges = (products) => {
  ...
  return `• Giá thấp nhất: ${formatPrice(min)}\n` +
