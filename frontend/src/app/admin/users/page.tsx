...
export default function AdminUsersPage() {
  ...
  const handleDelete = async (userItem: User) => {
    ...
    if (
      !confirm(
        `Bạn có chắc muốn xóa người dùng ${userItem.email}? Hành động này không thể hoàn tác!`,
      )
    )
      return;

    try {
      await adminAPI.users.delete(userItem.id);
      toast.success("Xóa người dùng thành công!");
      loadUsers(pagination.page);
    } catch (error: any) {
      console.error("Error deleting user:", error);
      toast.error(
        error?.response?.data?.message || "Không thể xóa người dùng này!",
      );
    }
  };
  ...
  const adminUsers = users.filter((u) => isAdmin(u.email));
  const regularUsers = users.filter((u) => !isAdmin(u.email));
            ...
            {list.map((userItem) => (
              ...
              >
                ...
                <td className="px-5 py-4 text-gray-700 max-w-[180px]">
                  {userItem.address ? (
                    <span title={userItem.address} className="block truncate">
                      {userItem.address}
                    </span>
                  ) : (
                    <span className="text-gray-400 italic">Chưa cập nhật</span>
          ...
          <div>
            ...
            {renderUserTable(
              ...
              search
                ...
                : "Không có tài khoản người dùng trong trang này",
            )}
          </div>
